...
class CentralBank:
    ...
    def setNominalInterestRate(self):
        ...
        self.economy.nominalInterestRate = (1+Parameters.InflationTarget)*(1+Parameters.NaturalInterestRate)*((1+self.economy.goodsMarket.getPrevInflation())/(1+Parameters.InflationTarget))**Parameters.Phi_inflation*((1+Parameters.NaturalUnemploymentRate)/(1+self.economy.labourMarket.prevUnemploymentRate))**Parameters.Phi_unemployment-1
        
        if self.economy.nominalInterestRate < -0.005:
            self.economy.nominalInterestRate = -0.005